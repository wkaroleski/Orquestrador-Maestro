---
name: linear-Codex-skill
description: "Manage Linear issues, projects, and teams"
allowed-tools: 
- WebFetch(domain: linear.app)
source: "https://github.com/wrsmith108/linear-Codex-skill"
risk: safe
---
...
**CRITICAL**: Never expose API keys in terminal output or Codex's context.
...
```bash
# ❌ NEVER - exposes key to Codex's context
linear config show
echo $LINEAR_API_KEY
...
```bash
npx tsx ~/.Codex/skills/linear/scripts/setup.ts
```
...
# Option B: Add to Codex environment
echo 'LINEAR_API_KEY=lin_api_your_key_here' >> ~/.Codex/.env

# Then reload your shell or restart Codex
```
...
```bash
npx tsx ~/.Codex/skills/linear/scripts/query.ts "query { viewer { name } }"
```
