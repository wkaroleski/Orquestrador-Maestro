...
| Codex 3.5 Sonnet | $$ | Fast | Excellent | Balanced |
| Codex 3 Haiku | $ | Fastest | Good | High volume |
```
...
function calculateCost(usage) {
  const rates = {
    'Codex-3-haiku': { input: 0.25, output: 1.25 }, // per 1M tokens
  };
  const rate = rates['Codex-3-haiku'];
