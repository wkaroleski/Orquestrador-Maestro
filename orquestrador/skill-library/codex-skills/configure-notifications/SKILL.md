...
- If follow-up is needed, explicitly instruct clawdbot to consult the local project context file and continue in the configured Discord channel.
...
- **Reply target format**: Use `--reply-to 'channel:<DISCORD_CHANNEL_ID>'` for reliability (preferred over channel aliases).

Example (replace `<DISCORD_CHANNEL_ID>` and any project context filename before use):

```bash
jq \
  --arg command "(clawdbot agent --session-id omx-hooks --message {{instruction}} --thinking minimal --deliver --reply-channel discord --reply-to 'channel:<DISCORD_CHANNEL_ID>' --timeout 120 --json >>/tmp/omx-openclaw-agent.jsonl 2>&1 || true)" \
  '.notifications = (.notifications // {enabled: true}) |
   .notifications.enabled = true |
   ...
   .notifications.openclaw.hooks["session-start"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMX hook=session-start project={{projectName}} session={{sessionId}} tmux={{tmuxSession}}. 한국어로 상태를 공유하고 로컬 프로젝트 컨텍스트를 참고해 필요한 후속 조치를 설정된 Discord 채널에 안내하세요."
   } |
   ...
   .notifications.openclaw.hooks["ask-user-question"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMX hook=ask-user-question session={{sessionId}} tmux={{tmuxSession}} question={{question}}. 한국어로 사용자 응답 필요를 설정된 Discord 채널에 알리고 즉시 액션 아이템을 제시하세요."
   } |
   .notifications.openclaw.hooks["stop"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMX hook=session-stop project={{projectName}} session={{sessionId}} tmux={{tmuxSession}}. 한국어로 중단 상태와 정리 액션을 로컬 프로젝트 컨텍스트 기준으로 전달하세요."
   } |
...
clawdbot agent --session-id omx-hooks --message "OMX hook test via clawdbot agent path" \
  --thinking minimal --deliver --reply-channel discord --reply-to 'channel:<DISCORD_CHANNEL_ID>' --timeout 120 --json
```
